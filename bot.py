...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "📌 *Все команды бота:*\n\n"
        "/start — подписаться на ежедневные уроки\n"
        "/now — получить полный урок прямо сейчас\n"
        "/python — только задача по Python\n"
        "/words — только английские слова\n"
        "/fact — только интересный факт\n"
        "/quote — только цитата дня\n"
        "/done — отметить день выполненным 🎉\n"
        "/stop — отписаться от рассылки\n"
        "/help — показать это меню\n\n"
        f"📊 *В базе:*\n"
        f"🐍 {len(PYTHON_TASKS)} задач по Python\n"
        f"🇬🇧 {len(ENGLISH_WORDS)} английских слов\n"
        f"✦ {len(QUOTES)} цитат\n"
        f"💡 {len(FACTS)} интересных фактов",
        parse_mode="Markdown"
    )
...
async def cmd_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msgs = ["🎉 Молодец! Ещё один день учёбы позади!","⭐ Отлично! Ты не остановился — это главное!","🔥 Стрик продолжается! Каждый день — шаг вперёд!","💪 Умница! Маленькие шаги каждый день = большой результат!","🚀 Так держать! Ты инвестируешь в своё будущее!"]
    await update.message.reply_text(msgs[date.today().toordinal() % len(msgs)])
...
def main():
    ...
    for cmd, fn in [("start",start),("help",cmd_help),("now",cmd_now),
                    ...
                    ("quote",cmd_quote),("done",cmd_done),("stop",cmd_stop)]:
